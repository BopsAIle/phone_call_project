...
- ~~Chỉ sửa trong thư mục `AI/`~~ → **đã mở khóa BE**: thêm Phần D lưu trữ cuộc gọi + transcript.
...
## Phần D — Lưu trữ cuộc gọi và transcript (bổ sung, có sửa BE)

**Vấn đề:** BE hiện có `restaurants / branches / bookings / menu` nhưng **không có gì lưu cuộc gọi**. Khách
khiếu nại "tôi đâu có đặt món này" thì không có gì đối chiếu. Cũng không xem lại được AI nói sai ở đâu.

### D1. BE — hai bảng mới

TypeORM đang bật `synchronize: true` nên bảng tự tạo, **không cần viết migration**.

- **`calls`** — một dòng mỗi cuộc gọi: `call_id` (mã từ nhà mạng, có unique index), `restaurant_id`,
  `branch_id`, số gọi đến / gọi đi, thời điểm bắt đầu / kết thúc, thời lượng, trạng thái, ý định
  (đặt bàn / mang về / giao hàng), và liên kết tới `booking_id` nếu cuộc gọi tạo ra đơn.
- **`call_messages`** — mỗi lượt nói một dòng: thuộc cuộc gọi nào, thứ tự, vai (`user` / `assistant` /
  `tool`), nội dung, tên tool nếu có, thời điểm.

Module `calls/` theo đúng pattern của `bookings/`: entity → repository → service → controller → module.
Ba endpoint cho AI dùng:

| Endpoint | Lúc nào | Việc |
|---|---|---|
| `POST /calls/ai/start` | đầu cuộc gọi | tạo dòng `calls`, trả về id. Gọi lại cùng `call_id` thì trả về dòng cũ (không tạo trùng) |
| `POST /calls/ai/:id/messages` | định kỳ trong cuộc gọi | thêm một lô lượt nói |
| `POST /calls/ai/:id/end` | kết thúc | ghi thời điểm kết thúc, thời lượng, trạng thái, lô lượt nói còn lại |

Thêm cả endpoint đọc cho dashboard: `GET /calls`, `GET /calls/:id` (kèm transcript).

**Lưu ý cấu hình BE:** `ValidationPipe` đang bật `forbidNonWhitelisted: true` — gửi thừa một trường là 400.
DTO phải khớp chính xác với thứ AI gửi.

### D2. AI — client + bộ gom transcript

- `calls/client.py`: HTTP client, cùng kiểu với `booking/client.py` / `order/client.py`.
- `calls/recorder.py`: gom lượt nói trong RAM, đẩy lên BE **theo lô** — mặc định 30 giây một lần hoặc khi
  đủ 10 lượt, và đẩy nốt lúc cúp máy.

**Nguyên tắc quan trọng: việc lưu trữ không bao giờ được làm chậm hay làm hỏng cuộc gọi.** Mọi lần gọi BE
chạy ở task nền, mọi lỗi chỉ ghi log rồi bỏ qua. `RESTAURANT_API_BASE` sập thì cuộc gọi vẫn chạy bình
thường, chỉ là không có transcript.

**Lấy nội dung ở đâu:** không lấy thẳng `session.history` (nó lẫn JSON của tool, dài và khó đọc). Lấy từ hai
chỗ đã có sẵn:
- câu khách nói: [`on_transcript_completed`](AI/bridge/session.py#L746)
- câu AI nói: [`note_spoken`](AI/bridge/session.py#L168) — hàm này chỉ ghi khi byte tiếng đầu tiên **đã ra
  dây**, nên transcript phản ánh đúng thứ khách thật sự nghe, không phải thứ model định nói rồi bị ngắt.

Bật/tắt bằng `CALL_LOG_ENABLED` (mặc định bật, tắt được để chạy test).

### D3. Tiện thể: chống tạo đơn trùng khi lỗi mạng

Đây là thứ mục "không làm" cũ đã chặn vì cần BE, giờ mở khóa được. Khi gửi đơn bị timeout, BE **có thể đã
ghi đơn rồi** mà AI không biết — thử lại là ra hai đơn.

Cho BE nhận `call_id` làm khóa chống trùng ở `POST /bookings/ai` và `POST /menu/*/ai`: cùng `call_id` thì
trả về đơn đã tạo thay vì tạo đơn mới. Có nó rồi thì phía AI mới thử lại an toàn được —
`documents/new_pipeline.md:1418` ghi đúng điều kiện này: *"Muốn retry an toàn thì backend phải hỗ trợ
idempotency key (ví dụ nhận `callId` làm khóa chống trùng) — chưa có thì đừng retry."*
...
Đợt 1b — lưu trữ cuộc gọi (sau A2, vì cùng đụng vòng đời session)
  D1. BE: bảng calls + call_messages, module calls/
  D2. AI: calls/client.py + calls/recorder.py, nối vào session
  D3. call_id làm khóa chống trùng đơn
